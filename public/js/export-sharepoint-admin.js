...
function render() {
  ...
  mountedContainer.innerHTML = `
    <div class="stack">
      <p class="hint">Un rapport PDF des données (stock central, stock par site, historique des inventaires, sorties de stock par site, interventions, fiches de traçabilité ménage, relevés de compteurs) est envoyé automatiquement vers SharePoint à la connexion, une fois par jour maximum — en plus du stockage principal dans l'appli, pas à la place. Chaque module a son propre sous-dossier dans "ExportsDonnees" (Stock, Interventions, Menage, Compteurs). Pour les compteurs : un récapitulatif global, un détail par site, et un PDF individuel par compteur (avec courbe d'évolution et historique découpé par année scolaire) rangé par type — "Compteurs/[Site]/Eau/", "/Gaz/", "/Électricité/". Un sous-dossier "Archives" est créé à chaque niveau pour les copies datées, si les données ont changé depuis le dernier export. Nécessite qu'une session Microsoft soit déjà active dans le navigateur pour se déclencher tout seul.</p>
